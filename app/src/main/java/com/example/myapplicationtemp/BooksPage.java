...
/*
Class for activity to choose page ex for book
 */
public class BooksPage extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    ...
    public void OpenExclass() {
        ...
        if (pageSend.isEmpty()) {
            page.setError("page NO. is required");
            page.requestFocus();
            return;
        }
        if (Integer.parseInt(pageSend)>0) {
            page.setError("page NO. is required");
            page.requestFocus();
            return;
        }
        if (exSend.isEmpty()) {
            ex.setError("Ex NO. is required");
            ex.requestFocus();
            return;
        }
        if (Integer.parseInt(exSend)>0) {
            ex.setError("Ex NO. is required");
            ex.requestFocus();
            return;
        }
